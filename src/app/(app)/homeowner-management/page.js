...
import { buildHomeownerMasterlistHtml, buildHomeownerMasterlistCsv } from '@/lib/homeownerMasterlistTemplate'
...
const mapRecordToHomeowner = (record = {}, paymentSummary = null) => {
  ...
  return {
    ...
    household_members: householdMembersValue,
    householdNo: record.household_no,
    residentId: generatedId || '-',
    workStatus: String(record.work_status || '-'),
    work_status: String(record.work_status || '-'),
...
function HomeownerManagementInner() {
  ...
  const exportToCsv = () => {
    ...
    const csv = buildHomeownerMasterlistCsv({ homeowners: filteredHomeowners })
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    const dateStr = new Date().toISOString().slice(0, 10)
    link.setAttribute('href', url)
    link.setAttribute('download', `homeowner-masterlist-${dateStr}.csv`)
    ...
    notify.success({
      ...
      description: `Exported masterlist for ${filteredHomeowners.length} homeowners as CSV.`
    })
  }
      ...
      <div className={styles.pageContent}>
        ...
        <div className={styles.controlsRow}>
          ...
          <div className={styles.headerActions}>
            ...
            <button type="button" className={styles.exportCsvButton} onClick={exportToCsv}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18M9 21V9" /></svg>
              Export CSV
            </button>
