...
const buildReceiptPreview = (_dateValue, existingReceipts = []) => {
  // Global preview across all billing periods: one past the highest receipt.
  const rangeStart = 10000001
  const rangeEnd = 99999999

  let maxSeq = -1
  existingReceipts.forEach((value) => {
    const numeric = Number(String(value || '').replace(/\D/g, ''))
    if (Number.isInteger(numeric) && numeric >= rangeStart && numeric <= rangeEnd) {
      const seq = numeric - rangeStart
      if (seq > maxSeq) {
        maxSeq = seq
      }
    }
  ...
  const nextNumber = rangeStart + maxSeq + 1
  return nextNumber <= rangeEnd ? String(nextNumber).padStart(8, '0') : ''
}
...
export default function PaymentMonitoringPage() {
  ...
  const openRecordModal = () => {
    ...
    const receiptNo = buildReceiptPreview(today, records.map((record) => record.receiptNo))
  ...
  const savePaymentRecord = async ({ shouldPrint = false } = {}) => {
    ...
    const previewReceiptNo =
      form.receiptNo || buildReceiptPreview(paymentDate, records.map((record) => record.receiptNo))

    if (!form.recordId || !amountPaid) {
      notify.error({
    ...
    try {
      ...
      // The server is authoritative for the generated receipt number; use the
      // assigned number (falling back to the local preview when unavailable).
      const assignedReceiptNo = String(response?.payment?.receipt_no || previewReceiptNo)
